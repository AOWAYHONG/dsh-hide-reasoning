...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /* Reset the probe backoff when the composer's effort badge appears, so a
       late-mounting model selector is discovered promptly. Called from the
       global observer. */
    function maybeResetProbeBackoff(n) {
      if (probeFailStreak === 0) return;
      if (!n || !n.matches) return;
      if (n.matches(MODEL_EFFORT_SELECTORS.join(",")) ||
          (n.querySelector && n.querySelector(MODEL_EFFORT_SELECTORS.join(",")))) {
        probeFailStreak = 0;
        lastProbeAt = 0;
        lastProbeName = "";
      }
    ...
    var CSS = [
      ...
      ".hrx-body:focus-visible{outline:2px solid rgba(194,58,178,.4);outline-offset:-2px}",
    ...
    /* Lightweight i18n: match the app's locale (Chinese UI → Chinese
       labels). Falls back to English. */
    ...
    var I18N = L10N[detectLocale()] || L10N.en;
    ...
    /* Display names for real tool-call records from the conversation trail.
       DSH renders every real tool invocation as a flow item carrying
       `data-tool="<name>"`; unknown names pass through capitalized. */
    var TOOL_DISPLAY = {
      read: "Read", search: "Search", bash: "Bash", write: "Write",
      edit: "Edit", code: "Code", web: "Web", browser: "Browser",
      grep: "Grep", python: "Python", files: "Files", filesystem: "Files",
      subagent: "Subagent", git: "Git", curl: "curl", ffmpeg: "ffmpeg",
      shell: "Shell", task: "Task", goal: "Goal"
    };

    function displayToolName(name) {
      var n = String(name || "");
      if (TOOL_DISPLAY[n]) return TOOL_DISPLAY[n];
      return n ? n.charAt(0).toUpperCase() + n.slice(1) : n;
    }

    /* Read the REAL tool invocations from the conversation trail: the
       reasoning row's turn (flowItem) is followed by sibling tool-call
       flowItems (`data-chat-flow-kind="tool-call"`) whose cards carry
       `data-tool="<name>"`. Returns:
         - []        trail context found, zero tool-calls (authoritative "None")
         - [names]   distinct tool display names found
         - null      NO trail context (caller falls back to text mentions)
    */
    var TRAIL_MAX = 6;

    function detectToolsFromTrail(row) {
      if (!row || !row.closest) return null;
      var flow = row.closest('[data-chat-flow-key]');
      if (!flow || !flow.parentElement) return null;
      var tools = [];
      var sibling = flow.nextElementSibling;
      var guard = 0;
      while (sibling && guard < 40) {
        var kind = sibling.getAttribute ? sibling.getAttribute("data-chat-flow-kind") : null;
        if (kind === "tool-call") {
          /* Collect ALL tool cards, including recursive subcalls. */
          var cards = sibling.querySelectorAll('[data-tool]');
          for (var c = 0; c < cards.length; c++) {
            var name = cards[c].getAttribute("data-tool");
            var d = displayToolName(name);
            if (d && tools.indexOf(d) < 0 && tools.length < TRAIL_MAX) tools.push(d);
          }
        } else if (kind === "assistant-step" || kind === "assistant-text" || kind === "text" || kind === "assistant") {
          /* next reasoning/final answer — the tool window ends */
          break;
        }
        sibling = sibling.nextElementSibling;
        guard++;
      }
      return tools; /* [] means "trail found, no tools" — authoritative */
    }

    /* Module-level constant arrays — don't re-allocate per call. */
    var TOOL_ORDER = ["search", "python", "bash", "shell", "grep", "curl", "ffmpeg", "browser", "subagent", "filesystem"];
    var TOOL_MENTION_NAMES = { search: "Search", python: "Python", bash: "Bash", shell: "Shell", grep: "Grep", curl: "curl", ffmpeg: "ffmpeg", browser: "Browser", subagent: "Subagent", filesystem: "Files" };
    ...
    function detectTools(t) {
      ...
      for (var i = 0; i < TOOL_ORDER.length; i++) {
        if (hits[TOOL_ORDER[i]]) out.push(TOOL_MENTION_NAMES[TOOL_ORDER[i]]);
    ...
    function buildCard() {
      ...
      text(leftName, I18N.reasoningLabel);
      ...
      var rows = [
        [I18N.context, "hrx-context"],
        [I18N.reasoningLabel, "hrx-stages"],
        [I18N.tools, "hrx-tools"],
        [I18N.completion, "hrx-completion"]
      ];
      var fields = {};
      ...
      /* a11y: keyboard-scrollable reasoning body + live region for streaming
         stat updates. */
      body.tabIndex = 0;
      body.setAttribute("role", "region");
      body.setAttribute("aria-label", I18N.reasoningLabel);
    ...
    function refresh(row, rec) {
      ...
      /* Real tool calls from the conversation trail first; fall back to
         text-mention detection ONLY when no trail context exists (null).
         An empty trail [] is authoritative: no tools were called.
         The trail is static once the turn completes, so cache by the
         presence of the full body / running state to avoid re-walking the
         DOM on every 150ms refresh. */
      var tools = null;
      if (rec.lastToolsKey === "" || (hasBody && !running)) {
        var trailTools = detectToolsFromTrail(row);
        tools = trailTools !== null ? trailTools : detectTools(t);
        rec.lastToolsKey = (hasBody ? "b" : "s") + (running ? "r" : "d") + ":" + tools.join(",");
      } else {
        tools = rec.lastToolsKey.split(":")[1] ? rec.lastToolsKey.split(":")[1].split(",") : [];
      }
      ...
      if (logo !== rec.lastLogo) {
        ...
        text(rec.card.leftName, logo ? logo.name : I18N.reasoningLabel);
      }

      text(rec.card.badgeTokens, fmtCount(tokens));
      text(rec.card.context, fmtCount(tokens) + " " + I18N.tokens);
      text(rec.card.stages, stages > 0 ? stages + " " + I18N.stage1 + (stages > 1 ? "s" : "") : "—");
      text(rec.card.tools, tools.length ? tools.join(" · ") : I18N.none);
      ...
      if (running) {
        ...
        text(rec.card.rightName, I18N.streaming);
        text(rec.card.badgeElapsed, I18N.thinking);
        text(rec.card.completion, "…");
      } else {
        if (rec.startAt !== null && rec.doneAt === null) rec.doneAt = Date.now();
        text(rec.card.rightName, I18N.done);
    ...
    function attach(row) {
      ...
      var obs = new MutationObserver(function (muts) {
        ...
        /* Retry opening the native disclosure: the row may have been
           ...
           or React may have re-rendered the toggle since. Once confirmed
           open, stop re-querying on every streaming mutation. */
        if (rec.openAttempts < 5 && !rec.nativeOpened) {
          var openRow = row.querySelector('[data-disclosure-row]');
          if (openRow) {
            if (openRow.getAttribute("aria-expanded") === "true") {
              rec.nativeOpened = true;
            } else {
              rec.openAttempts++;
              tryOpenNative(row);
            }
      ...
      /* Degraded fallback: if the full body still can't be mounted after a
         ...
         reasoning is never hidden with no readable copy. Never abandon a
         still-streaming row (data-state="running") — re-arm instead. */
      rec.keepTimer = setTimeout(function keepCheck() {
        if (!row.isConnected || !registry.has(row)) return;
        if (row.querySelector(BODY_SELECTOR)) return; /* full body arrived */
        if (row.getAttribute("data-state") === "running") {
          /* still streaming — give it more time, re-arm once */
          if (rec.openAttempts < 5) {
            rec.openAttempts++;
            tryOpenNative(row);
          }
          rec.keepTimer = setTimeout(keepCheck, 2500);
          return;
        }
        var openRow = row.querySelector('[data-disclosure-row]');
    ...
    /* Semantic-version comparison: "0.4.1" > "0.4.0". Returns >0, 0, <0. */
    ...
    function start() {
      ...
      if (prev) {
        if (compareVersions(prev.version, PLUGIN_VERSION) > 0) return; /* newer instance already active */
      ...
      observer = new MutationObserver(function (muts) {
        for (var i = 0; i < muts.length; i++) {
          ...
          for (j = 0; j < m.addedNodes.length; j++) {
            ...
            maybeResetProbeBackoff(n);
            if (n.matches && n.matches(ATTR)) attach(n);
            q = n.querySelectorAll ? n.querySelectorAll(ATTR) : [];
            for (k = 0; k < q.length; k++) attach(q[k]);
            /* A tool-call flow item just appeared: refresh the reasoning row
               of the same turn so its Tools stat picks up the real record.
               Resolve the tool-call FLOW ITEM itself (descendant of n in
               the bulk-insert case), then walk back to the assistant-step. */
            if (n.matches && (n.matches('[data-chat-flow-kind="tool-call"]') || n.querySelector('[data-chat-flow-kind="tool-call"]'))) {
              var toolFlow = null;
              if (n.matches('[data-chat-flow-key][data-chat-flow-kind="tool-call"]')) {
                toolFlow = n;
              } else if (n.querySelector) {
                toolFlow = n.querySelector('[data-chat-flow-key][data-chat-flow-kind="tool-call"]');
              }
              if (toolFlow && toolFlow.parentElement) {
                var prev = toolFlow.previousElementSibling;
                var g = 0;
                while (prev && g < 40) {
                  var pk = prev.getAttribute ? prev.getAttribute("data-chat-flow-kind") : null;
                  if (pk === "assistant-step") {
                    var thinkRow = prev.querySelector(ATTR);
                    if (thinkRow && registry.has(thinkRow)) queueRefresh(thinkRow);
                    break;
                  }
                  if (pk === "assistant-text" || pk === "text" || pk === "assistant") break;
          ...
          for (j = 0; j < m.removedNodes.length; j++) {
            ...
            /* A tool-call card was removed: refresh its turn's reasoning so
               stale tool names don't linger. */
            if (n.matches && (n.matches('[data-chat-flow-kind="tool-call"]') || (n.querySelector && n.querySelector('[data-chat-flow-kind="tool-call"]')))) {
              var removedFlow = n.matches('[data-chat-flow-key][data-chat-flow-kind="tool-call"]')
                ? n
                : (n.querySelector ? n.querySelector('[data-chat-flow-key][data-chat-flow-kind="tool-call"]') : null);
              if (removedFlow && removedFlow.parentElement) {
                var rp = removedFlow.previousElementSibling;
                var rg = 0;
                while (rp && rg < 40) {
                  var rk = rp.getAttribute ? rp.getAttribute("data-chat-flow-kind") : null;
                  if (rk === "assistant-step") {
                    var rThink = rp.querySelector(ATTR);
                    if (rThink && registry.has(rThink)) queueRefresh(rThink);
                    break;
                  }
                  if (rk === "assistant-text" || rk === "text" || rk === "assistant") break;
